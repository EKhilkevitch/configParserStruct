...
#include "configParserStruct2/bisonconstants.h"
...
%union 
{
  char String[CPSSPU_MAX_STRING_LENGTH];
  char IdName[CPSSPU_MAX_ID_LENGTH];
  int  IntNumber;
  double RealNumber;
  char CmpOpStr[CPSSPU_MAX_CMP_LENGTH];
}
...
%token <IntNumber>  TOKEN_ARGUMENT
...
program        : statementSeq { CPSSPU_endOfProgram(); }
...
return         : TOKEN_RETURN expression delimiter { CPSSPU_returnFromCurrentFunction(); }
               | TOKEN_RETURN { CPSSPU_pushDictToStack();  } '{' dictFields    '}' delimiter { CPSSPU_returnFromCurrentFunction(); }
               | TOKEN_RETURN { CPSSPU_pushArrayToStack(); } '[' arrayElements ']' delimiter { CPSSPU_returnFromCurrentFunction(); }
               ;

function       : idRef '=' TOKEN_FUNCTION { CPSSPU_beginOfNewFunctionAssignName(); } '{' statementSeq '}' { CPSSPU_endOfCurrentFunction(); }
               ;
...
expression     : exprSet    
               | idRef '=' { CPSSPU_pushDictToStack();  } '{' dictFields    '}'{ CPSSPU_assignVariableValueFromStack(); }
...
arrayElementsList : arrayElementsList ',' expression { CPSSPU_setArrayElementFromStack(); }
               | arrayElementsList ',' { CPSSPU_pushDictToStack(); }  '{' dictFields    '}' { CPSSPU_setArrayElementFromStack(); }
...
dictField      : '.' TOKEN_ID '=' exprSet { CPSSPU_setDictFieldFromStack($2); }
               | '.' TOKEN_ID '=' { CPSSPU_pushDictToStack(); }  '{' dictFields    '}' { CPSSPU_setDictFieldFromStack($2); }
               | '.' TOKEN_ID '=' { CPSSPU_pushArrayToStack(); } '[' arrayElements ']' { CPSSPU_setDictFieldFromStack($2); }
               ;

idRef          : idRef '.' TOKEN_ID                { CPSSPU_setDictKeyString( $3 ); }
               | idRef '[' expression ']'          { CPSSPU_setArrayIndexFromTop(); }
               | idRef '{' expression '}'          { CPSSPU_setDictKeyFromTop(); }
               | TOKEN_ID                          { CPSSPU_pushVariableReferenceToStack( $1 ); }
               | TOKEN_ARGUMENT                    { CPSSPU_pushArgumentReferenceToStack( $1 ); }
               | TOKEN_SYMREF_BEGIN expression '}' { CPSSPU_pushVariableSymbolicReferenceToStack(); }
               | TOKEN_GLOBAL TOKEN_ID             { CPSSPU_pushVariableGlobalReferenceToStack( $2 ); } 
               ;
